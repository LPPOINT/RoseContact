...
namespace AdvancedInspector
{
    public class EnumEditor : FieldEditor
    {
        ...
        public override void Draw(InspectorField field, GUIStyle style)
        {
            ...
            if (display == null || !display.Masked)
            {
                if (display == null || display.Display == EnumDisplay.DropDown)
                    value = DrawDropDown(field.Type, (int)value, style, false);
                else if (display.Display == EnumDisplay.Button)
                    value = DrawEnum(field.Type, (int)value, display.MaxItemsPerRow, style == null ? EditorStyles.toolbarButton : style);
                else if (display.Display == EnumDisplay.Checkbox)
                    value = DrawEnum(field.Type, (int)value, display.MaxItemsPerRow, style == null ? EditorStyles.toggle : style);
            }
            else
            {
                if (display == null || display.Display == EnumDisplay.DropDown)
                    value = DrawDropDown(field.Type, (int)value, style, true);
                else if (display.Display == EnumDisplay.Button)
                    value = DrawMasked(field.Type, (int)value, display.MaxItemsPerRow, style == null ? EditorStyles.toolbarButton : style);
                else if (display.Display == EnumDisplay.Checkbox)
                    value = DrawMasked(field.Type, (int)value, display.MaxItemsPerRow, style == null ? EditorStyles.toggle : style);
            }
        ...
        private int SelectedIndex(Array values, int value)
        {
        ...
        private object DrawDropDown(Type type, int value, GUIStyle style, bool masked)
        {
            ...
            {
                if (style == null)
                    value = EditorGUILayout.MaskField(value, names);
                else
                    value = EditorGUILayout.MaskField(value, names, style);
        ...
        private object DrawEnum(Type type, int value, int max, GUIStyle style)
        {
            if (max < 1)
                max = 6;
        ...
        private object DrawMasked(Type type, int value, int max, GUIStyle style)
        {
            if (max < 1)
                max = 6;
